...
headers = ['frame' , 'rea' , 'rwa' , 'rsa', 'rha' , 'rka' , 'lea' , 'lwa' , 'lsa', 'lha' , 'lka' , 'neckFlex' , 'neckLatFlex']
data = []
...
while cap.isOpened() == True:
    ...
    try:
        ...
        lEar = [lm[mp_pose.PoseLandmark.LEFT_EAR.value].x , lm[mp_pose.PoseLandmark.LEFT_EAR.value].y]
        rEar = [lm[mp_pose.PoseLandmark.RIGHT_EAR.value].x , lm[mp_pose.PoseLandmark.RIGHT_EAR.value].y]
        lEye =  [lm[mp_pose.PoseLandmark.LEFT_EYE.value].x , lm[mp_pose.PoseLandmark.LEFT_EYE.value].y]
        rEye =  [lm[mp_pose.PoseLandmark.RIGHT_EYE.value].x , lm[mp_pose.PoseLandmark.RIGHT_EYE.value].y]
        ...
        headMid = [(lEar[0] + rEar[0])/2 , (lEar[1] + rEar[1])/2]
        hipMid = [(lHip[0] + rHip[0])/2 , (lHip[1] + rHip[1])/2]
        # alpha = 1
        neckLow = [(lShoulder[0] + rShoulder[0])/2 , (((lShoulder[1] + rShoulder[1])/2))]
        eyeMid = [(lEye[0] + rEye[0])/2 , (lEye[1] + rEye[1])/2]
        shoulderUp = [(lShoulder[0] + rShoulder[0])/2 , 0]


        # neckFlexAng = ang(nose, neckLow, hipMid)
        neckLatFlexAng = ang(headMid , neckLow , shoulderUp)
        neckFlexAng = ang(headMid , neckLow , shoulderUp)
        ...
        if (sys.argv[3] == 'f'):
            ...
            cv2.putText(img , str(neckFlexAng) , tuple(np.multiply(neckLow , [frame_width , frame_height]).astype(int)) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (255,255,255) , 2 , cv2.LINE_AA)
        elif (sys.argv[3] == 'l'):
            cv2.putText(img , str(neckLatFlexAng) , tuple(np.multiply(neckLow , [frame_width , frame_height]).astype(int)) , cv2.FONT_HERSHEY_SIMPLEX , 1 , (255,255,255) , 2 , cv2.LINE_AA)

        d = [fNum , rElbowAng , rWristAng , rShoulderAng , rHipAng , rKneeAng , lElbowAng , lWristAng , lShoulderAng , lHipAng , lKneeAng , neckFlexAng , neckLatFlexAng]
        data.append(d)
